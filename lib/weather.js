...
function getWeatherLocation(location, callback) {
    ...
    rp(options)
        .then((data) => {
            if (data.query.results !== null && data.query.results.channel) {
                callback(parseWeatherInfo(data.query.results.channel));
            } else {
                callback('Hmmm...I can\'t seem to find the location, "' + loc + '". 😕');
            }
        })
        .catch((err) => {
            console.log(err);
